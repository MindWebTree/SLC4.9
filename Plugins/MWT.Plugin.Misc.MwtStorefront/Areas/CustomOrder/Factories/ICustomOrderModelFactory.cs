...
namespace MWT.Plugin.Misc.MwtStorefront.Areas.CustomOrder.Factories
{
    public partial interface ICustomOrderModelFactory
    {
        ...
        Task<CustomOrderModel> PrepareCustomerOrderModel(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order);
        ...
        Task<bool> IsOrderPaid(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order);
        Task<bool> ProductSearchEnableDisable(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order);
        Task<bool> CustomerSearchEnableDisable(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order);
        ...
        Task<(decimal wgsCharges, decimal defaultWgsCharges, decimal surchargeAmount)> GetWgsCharges(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order, decimal subTotal, bool isSurchargeApplicable = false);
        ...
        Task<AdditionalServiceModel> PrepareAdditionalServiceModel(AdditionalServiceModel model,
              MWT.Nop.Core.Domain.CustomOrders.CustomOrder Customorder = null,
            Order order = null,

            MWT.Nop.Core.Domain.CustomOrders.OrderStatus status = MWT.Nop.Core.Domain.CustomOrders.OrderStatus.SavedDraft);
        Task<(Dictionary<int, dynamic> pairedOrders, string validPairedOrders, decimal orderSubTotal, decimal orderTotal, bool isSurchargeApplicable)> GetAdditionalServiceOrderInfo(MWT.Nop.Core.Domain.CustomOrders.OrderStatus status, Order order, MWT.Nop.Core.Domain.CustomOrders.CustomOrder customOrder, string pairedOrderIds);

        Task AdditionalServiceSendInvoice(MWT.Nop.Core.Domain.CustomOrders.CustomOrder order, decimal previousWgsAmount, decimal previousWgsDiscount, decimal currentWgsAmount, decimal currentWgsDiscount, decimal defaultWgsCharges = 0, decimal surchargeAmount = 0);
