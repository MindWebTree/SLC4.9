...
namespace MWT.Plugin.Misc.MwtStorefront.Factories.Catalog
{
    ...
    public partial class CustomProductModelFactory : ProductModelFactory, ICustomProductModelFactory
    {
        ...
        private readonly IVariantModelFactory _variantModelFactory;
                ...
                bool prepareAlternatePictureModel = false, bool isCategorypage = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (preparePictureModel && !prepareAlternatePictureModel)
                {
                    ...
                    var picture = await PrepareCustomProductOverviewPictureModelAsync(product, productThumbPictureSize, isCategorypage);
                    model.DefaultPictureModel = picture;
                }
...
bool prepareAlternatePictureModel = false, int variantId = 0)
        {
            ...
            foreach (var product in products)
            {
                var model = new CustomProductOverviewModel
                {
                    ...
                    DefaultVariantId = await _variantService.GetProductVariantId(product.Id)
                };
                ...
                if (variantId != 0)
                {
                    var variant = await _variantModelFactory.ValidateVariantID(product.Id, variantId);
                    if (variant == null)
                    {
                        variantId = 0;
                    }
                ...
                (model.DefaultPictureModel, allPictureModels) = await CustomPrepareProductDetailsPictureModelAsync(product, false, variantPictureId);
        ...
        protected virtual async Task<(string, IList<CustomProductDetailsModel.ProductAttributeModel>, int pictureId, List<List<int>> allCombinations)> CustomPrepareFeatureProductAttributeWithVariantTitleModelsAsync(Product product, ShoppingCartItem updatecartitem, int variantId = 0)
        {
 ...
 , product.Id, variantId, updatecartitem?.Id ?? 0, _webHelper.IsCurrentConnectionSecured(), await _storeContext.GetCurrentStoreAsync());
            ...
            {
                string variantTitle = "";
                var variantcombinations = await _variantService.GetProductVariants(product.Id);
                    ...
                    if (attribute.ShouldHaveValues())
                    {
                        ...
                        foreach (var attributeValue in _attributeValues)
                        {
                            attributeValues.Add(new ProductAttributeValue()
                            {
                                ...
                                PictureId = attributeValue.PictureId,
                        ...
                        else if (productAttrubute.Name.Equals(await _localizationService.GetResourceAsync("Product.Attr.Size"), StringComparison.InvariantCultureIgnoreCase))
                        {
                            ...
                            // need to confirm
                            //pictureId = attributeValues.Where(a => a.IsPreSelected).FirstOrDefault() != null ? attributeValues.Where(a => a.IsPreSelected).FirstOrDefault().FeaturedPictureId == 0 ?
                            //    attributeValues.Where(a => a.IsPreSelected).FirstOrDefault().PictureId : attributeValues.Where(a => a.IsPreSelected).FirstOrDefault().FeaturedPictureId : 0;

                            var preSelectedValue = attributeValues.FirstOrDefault(a => a.IsPreSelected);

                            variantTitle = preSelectedValue?.VariantTitle;

                            if (preSelectedValue != null)
                            {
                                var attributeValuePictures =
                                    await _customProductAttributeService.GetProductAttributeValuePicturesAsync(preSelectedValue.Id);

                                pictureId = preSelectedValue.FeaturedPictureId == 0
                                    ? attributeValuePictures.FirstOrDefault()?.PictureId ?? 0
                                    : preSelectedValue.FeaturedPictureId;
                            }
                            else
                            {
                                pictureId = 0;
                        ...
                        foreach (var attributeValue in attributeValues)
                        {
                            ...
                            //        attributeValue.PictureId, _webHelper.IsCurrentConnectionSecured(), await _storeContext.GetCurrentStoreAsync());
                            ...
                            {
                                var productAttributePictureCacheKey = _staticCacheManager.PrepareKeyForDefaultCache(NopModelCacheDefaults.ProductAttributePictureModelKey,
                                    attributeDefualtPictureId, _webHelper.IsCurrentConnectionSecured(), await _storeContext.GetCurrentStoreAsync());
                                var pictureModel = await _staticCacheManager.GetAsync(productAttributePictureCacheKey, async () =>
                                {
                                    var picture = await _pictureService.GetPictureByIdAsync(attributeDefualtPictureId);
                                    string fullSizeImageUrl, imageUrl, thumbImageUrl;
                                    ...
                                    return picture == null ? new CustomPictureModel() : new CustomPictureModel
                                    {
                                        FullSizeImageUrl = fullSizeImageUrl,
                                        ImageUrl = imageUrl,
                                        ThumbImageUrl = thumbImageUrl
                            ...
                            valueModel.PictureId = attributeDefualtPictureId;
                        }
                    }
        ...
        protected virtual async Task PrepareCustomGroupedProductOverviewPriceModelAsync(Product product, CustomProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            if (await _permissionService.AuthorizeAsync(StandardPermission.PublicStore.DISPLAY_PRICES))
            {
                ...
                foreach (var associatedProduct in associatedProducts)
                {
                    ...
                    var tierPrice = await _customProductService.GetTierPricesByProductAsync(associatedProduct.Id);
                    ...
                    if (tierPrice.Any())
                    {
                        ...
                        tmpMinPossiblePrice = Math.Min(tmpMinPossiblePrice,
                            (await _priceCalculationService.GetFinalPriceAsync(associatedProduct, await _workContext.GetCurrentCustomerAsync(), store, quantity: int.MaxValue)).Item1);
                    }
        ...
        protected virtual async Task PrepareCustomSimpleProductOverviewPriceModelAsync(Product product, CustomProductOverviewModel.ProductPriceModel priceModel)
        {
            ...
            priceModel.OldPriceValue = CustomCommonHelper.FormatPriceWithoutDecimal((decimal)priceModel.OldPriceValue);
        ...
        protected virtual async Task PrepareCustomProductOverviewPictureModelAsync(Product product, CustomProductOverviewModel model, int? productThumbPictureSize = null, bool isCategorypage = false)
        {
            ...
            {
                cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(CustomNopModelCacheDefaults.ProductDefaultPictureModelKey,
                ...
                var alternatePictureModel = await _staticCacheManager.GetAsync(cacheKey, async () =>
                {
                    ...
                    var _productPictureRepo = EngineContext.Current.Resolve<IRepository<ProductPicture>>();

                    var alterNatePictureRef = await _productPictureRepo.Table.Where(m => m.ProductId == product.Id && m.DisplayOrder == product.AlternateImageSequence).FirstOrDefaultAsync();
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                if (productAttribute.Name.CompareTo(attributeName) == 0)
                {
                    ...
                    foreach (var attrValue in attributeValues.OrderBy(m => m.DisplayOrder))
                    {
                        ...
                        {
                            var valuePictures = await _productAttributeService.GetProductAttributeValuePicturesAsync(attrValue.Id);
                            int pictureId = valuePictures.FirstOrDefault()?.PictureId ?? 0;

                            attributeModel.Add(new CustomProductOverviewModel.ProductAttributeModelWithImage()
                            {
                                Name = attrValue.Name,
                                PictureUrl = await this.CustomGetPicture(pictureId, productThumbPictureSize)
                            });
                        }
                    }
                    break;
          ...
          ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false, int variantId = 0)
        {
            ...
            var productVideo = await _videoService.GetVideosByProductIdAsync(product.Id);
            //standard properties
            var model = new CustomProductDetailsModel
            {
                ...
                VideoUrl = productVideo.FirstOrDefault().VideoUrl,
                VideoThumnailDisplayOrder = 2,
                Variants = await _variantService.GetPublishedProductVariants(product.Id),
            ...
            IList<CustomPictureModel> allPictureModels;
            (model.DefaultPictureModel, allPictureModels) = await CustomPrepareProductDetailsPictureModelAsync(product, isAssociatedProduct, variantPictureId);
            model.PictureModels = allPictureModels;
            ...
            //tier prices
            if (tierPrices.Any() && await _permissionService.AuthorizeAsync(StandardPermission.PublicStore.DISPLAY_PRICES))
            {
        ...
        protected virtual async Task<CustomProductOverviewModel.ProductPriceModel> PrepareCustomProductOverviewPriceModelAsync(Product product, int variantId = 0)
        {
            ...
            model.PriceValue = CustomCommonHelper.FormatPriceWithoutDecimal((decimal)model.PriceValue);
            model.OldPrice = CustomCommonHelper.FormatCurrencyPriceWithoutDecimal(model.OldPrice);
            model.OldPriceValue = CustomCommonHelper.FormatPriceWithoutDecimal((decimal)model.OldPriceValue);
            model.MinPrice = CustomCommonHelper.FormatCurrencyPriceWithoutDecimal(model.MinPrice);
            model.MinPriceValue = CustomCommonHelper.FormatPriceWithoutDecimal(model.MinPriceValue);
            model.MinOldPrice = CustomCommonHelper.FormatCurrencyPriceWithoutDecimal(model.MinOldPrice);
            model.MinOldPriceValue = CustomCommonHelper.FormatPriceWithoutDecimal(model.MinOldPriceValue);
        ...
        protected async Task<(bool isVariantExist, decimal msrp, decimal oldPrice, decimal price)> GetVariantPrice(Product product, int variantId)
        {
            ...
            var variant = (await this._variantService.GetProductVariants(product.Id)).Where(v => v.VariantId == variantId).FirstOrDefault();
            if (variant != null)
            {
        ...
        private async Task<string> GetProductEstimatedDeliveryDate(int productId, int variantId, string estimatedDeliveryDate)
        {
            if (variantId != 0)
            {
                estimatedDeliveryDate = (await _variantService.GetProductVariants(productId)).Where(v => v.VariantId == variantId).FirstOrDefault()?.EstimatedDeliveryDate ?? estimatedDeliveryDate;
            }
        ...
        protected virtual async Task<ProductDetailsModel.ProductBreadcrumbModel> PrepareCustomProductBreadcrumbModelAsync(Product product)
        {
            ...
            int categoryId = -1;
            var _commonModelFactory = EngineContext.Current.Resolve<ICustomCommonModelFactory>();
            Category category = new Category();
            (string entityType, int entityId) = await _commonModelFactory.GetRefererDetails();
        ...
        protected virtual async Task<IList<ProductTagModel>> CustomPrepareProductTagModelsAsync(Product product)
        {
            ...
            var thankGivingTag = await _settingService.GetSettingByKeyAsync<string>("ThanksGiving.ProductTag.Name");
            if (model.Where(t => t.Name.Equals(thankGivingTag, StringComparison.InvariantCultureIgnoreCase)).Any())
            {
                model = model.Where(t => t.Name.Equals(thankGivingTag, StringComparison.InvariantCultureIgnoreCase)).ToList();
            }
            else
            {
                var exludeTags = await _settingService.GetSettingByKeyAsync<string>("Product.Exclude.Tag");
                if (exludeTags != null)
                {
                    model = model.Where(t => !exludeTags.Split(',')
                                            .Any(excludeTag => string.Equals(t.Name, excludeTag, StringComparison.OrdinalIgnoreCase)))
                   .ToList();
                }
            }
            #endregion
        ...
        protected virtual async Task<(string, IList<CustomProductDetailsModel.ProductAttributeModel>, int pictureId, List<List<int>> allCombinations)> CustomPrepareProductAttributeWithVariantTitleModelsAsync(Product product, ShoppingCartItem updatecartitem, int variantId = 0)
        {
    ...
    , product.Id, variantId, updatecartitem?.Id ?? 0, _webHelper.IsCurrentConnectionSecured(), await _storeContext.GetCurrentStoreAsync());
            ...
            return await _staticCacheManager.GetAsync(productAttributeCacheKey, async () =>
            {
                ...
                string variantSize = "";
                var productpicture = await _pictureService.GetPicturesByProductIdAsync(product.Id);
                int pictureId = 0;
                if (product == null)
                    throw new ArgumentNullException(nameof(product));
                    ...
                    if (attribute.ShouldHaveValues())
                    {
                            ...
                            if (!productAttrubute.Name.Equals(await _localizationService.GetResourceAsync("Product.Attr.Stain"), StringComparison.InvariantCultureIgnoreCase))
                            {
                                foreach (var attributeValue in attributeValues)
                                {
                                    if (variantAttrs.Contains(attributeValue.Id))
                                    {
                                        ...
                                        {
                                            variantSize = attributeValue.Name;
                                            pictureId = productpicture.FirstOrDefault().Id;
                                        }
                                        attributeValue.IsPreSelected = true;
                                    }
                                    else
                        ...
                        foreach (var attributeValue in attributeValues)
                        {
                            ...
                            if (productpicture.Any())
                            {
                                ...
                                var pictureModel = await _staticCacheManager.GetAsync(productAttributePictureCacheKey, async () =>
                                {
                                    var picture = await _pictureService.GetPictureByIdAsync(productpicture.FirstOrDefault().Id);
                                    string fullSizeImageUrl, imageUrl, thumbImageUrl;
                            ...
                            if (await _permissionService.AuthorizeAsync(StandardPermission.PublicStore.DISPLAY_PRICES))
                            {
                                ...
                                var attributeValuePriceAdjustment = await _priceCalculationService.GetProductAttributeValuePriceAdjustmentAsync(product, attributeValue, customer, await _storeContext.GetCurrentStoreAsync());
                                var (priceAdjustmentBase, _) = await _taxService.GetProductPriceAsync(product, attributeValuePriceAdjustment);
                                var priceAdjustment = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(priceAdjustmentBase, await _workContext.GetWorkingCurrencyAsync());
                            ...
                            //picture of a product attribute value
                            valueModel.PictureId = productpicture.FirstOrDefault().Id;
                        }
                    }
        ...
        protected async Task<List<ProductAttributeCombination>> GetValidProductAttributeCombinationsAsync(Product product)
        {
        ...
        protected virtual async Task<(CustomPictureModel pictureModel, IList<CustomPictureModel> allPictureModels)> CustomPrepareProductDetailsPictureModelAsync(Product product, bool isAssociatedProduct, int pictureId = 0)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
        ...
        public async Task<List<ProductApiDetailsModel>> PrepareApiProductListModelAsync(int pageNumber, int productId)
        {
            ...
            if (productId == 0)
            {
                ...
                var products = await this._customProductService.SearchProductsAsync(pageIndex: pageNumber - 1, pageSize:
           ...
           ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false, int variantId = 0)
        {
            ...
            var model = new ProductApiDetailsModel
            {
                ...
                Gtin = product.Gtin,
                ManageInventoryMethod = product.ManageInventoryMethod,
                StockAvailability = await _customProductService.FormatStockMessageAsync(product, string.Empty),
                ParentGroupId = product.ParentGroupedProductId,
                EnableCustomizationModule = product.EnableCustomizationModule,
                Notes = product.Notes,
                TotalInventory = product.TotalInventory,
            ...
            IList<CustomPictureModel> allPictureModels;
            (model.DefaultPictureModel, allPictureModels) = await CustomPrepareProductDetailsPictureModelAsync(product, isAssociatedProduct);
            model.PictureModels = allPictureModels;
            ...
            #region Variants
            var variants = await _variantService.GetProductVariants(product.Id);
            ...
            model.Variants = await variants.SelectAwait(async variant =>
            {
                ...
                var attributes = await _productAttributeFormatter.FormatAttributesAsync(product, variant.Combination);
        ...
        { "Value", x[1].Trim() }
           ...
           .ToList();
            ...
            int variantPictureId = 0;
            (model.VariantTitle, model.ProductAttributes, variantPictureId) = await CustomPrepareApiProductAttributeModelsAsync(product, updatecartitem, variantId);
        ...
        public virtual async Task<IList<CustomProductSpecificationAttributeModel>> CustomfeedPrepareProductSpecificationAttributeModelAsync(Product product, SpecificationAttributeGroup group)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));
            ...
            var result = new List<CustomProductSpecificationAttributeModel>();
            ...
            foreach (var psa in productSpecificationAttributes)
            {
                ...
                if (model == null)
                {
                    ...
                    model = new CustomProductSpecificationAttributeModel
                    {
                        Id = attribute.Id,
        ...
        public async Task<string> GetProductMainImage(int productId)
        {
            ...
            return (await PrepareProductOverviewPicturesModelAsync(product, null))?.FirstOrDefault().FullSizeImageUrl ?? "";
        }
        ...
        protected virtual async Task<(string, IList<ProductApiDetailsModel.ProductAttributeModel>, int pictureId)> CustomPrepareApiProductAttributeModelsAsync(Product product, ShoppingCartItem updatecartitem, int variantId = 0)
        {
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in _attributeValues)
                    {
                        var attrivutevaluePictures = await _customProductAttributeService.GetProductAttributeValuePicturesAsync(attributeValue.Id);
                        int pictureid = attrivutevaluePictures.FirstOrDefault()?.Id ?? 0;
                        attributeValues.Add(new ProductAttributeValue()
                        {
                            AssociatedProductId = attributeValue.AssociatedProductId,
                            AttributeValueType = attributeValue.AttributeValueType,
                            ...
                            //       PictureId = attributeValue.PictureId,
                            PictureId = pictureid,
                            FeaturedPictureId = attributeValue.FeaturedPictureId == 0 ? pictureid : attributeValue.FeaturedPictureId,
                            PriceAdjustment = attributeValue.PriceAdjustment,
                            PriceAdjustmentUsePercentage = attributeValue.PriceAdjustmentUsePercentage,
                            ProductAttributeMappingId = attributeValue.ProductAttributeMappingId,
                            Published = attributeValue.Published,
                            ...
                            foreach (var attributeValue in attributeValues)
                            {
                                ...
                                if (variantAttrs.Contains(attributeValue.Id))
                                {
                                    if (productAttrubute.Name.Equals(await _localizationService.GetResourceAsync("Product.Attr.Size"), StringComparison.InvariantCultureIgnoreCase))
                                    {
                                        variantSize = attributeValue.Name;
                                        // need to condirm
                                        pictureId = attributeValuePictures.FirstOrDefault()?.PictureId ?? 0;
                                    }
                                    attributeValue.IsPreSelected = true;
                                }
                                else
                    ...
                    {
                        var attributepictures = await _customProductAttributeService.GetProductAttributeValuePicturesAsync(attributeValues.Where(a => a.IsPreSelected).FirstOrDefault().Id);
                        variantTitle = attributeValues.Where(a => a.IsPreSelected).FirstOrDefault()?.VariantTitle;
                        //  pictureId = attributeValues.Where(a => a.IsPreSelected).FirstOrDefault()?.PictureId ?? 0;
                        // need to confirm 
                        pictureId = attributepictures.FirstOrDefault()?.PictureId ?? 0;
                        variantSize = attributeValues.Where(a => a.IsPreSelected).FirstOrDefault()?.Name ?? "";
                    }
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (pictureid > 0)
                        {
                            ...
                            var pictureModel = await _staticCacheManager.GetAsync(productAttributePictureCacheKey, async () =>
                            {
                                ...
                                var picture = await _pictureService.GetPictureByIdAsync(attributepictures.FirstOrDefault()?.PictureId ?? 0);
                                string fullSizeImageUrl, imageUrl, thumbImageUrl;
                        ...
                        gallery = await _staticCacheManager.GetAsync(productPicturesCacheKey, async () =>
                        {
                            ...
                            string fullSizeImageUrl, imageUrl, thumbImageUrl;
                            // need to confirm
                            //     var pictureIds = await _customProductAttributeService.GetProductAttributeValueGalleryPicturesByAttributeValueId(attributeValue.Id);
                            var attributepictures = await _customProductAttributeService.GetProductAttributeValuePicturesAsync(attributeValue.Id);


                            List<Picture> pictures = new List<Picture>();
                            foreach (var pictureId in attributepictures.Select(x => x.PictureId))
                            {
                                if (pictureId != 0)
                                {
                                    var picture = await _pictureService.GetPictureByIdAsync(pictureId);
        ...
        //        IList<CustomProductSpecificationAttributeModel> customAttributes = Attributes
        //        .Select(a => new CustomProductSpecificationAttributeModel
        //        {
        //            Id = a.Id,
        //            Name = a.Name,
        //            Values = a.Values.Select(v => new ProductSpecificationAttributeValueModel
        //            {
        ...
        //            if (group.Id == 4)
        //            {
        //                customAttributes = await CustomfeedPrepareProductSpecificationAttributeModelAsync(product, group);
        //                foreach (var attr in Attributes)
        //                {
        ...
        //    return lstModel;
        //}
        //protected virtual async Task<(PictureModel pictureModel, IList<PictureModel> allPictureModels)> CustomPfeedrepareProductDetailsPictureModelAsync(Product product, bool isAssociatedProduct)
        //{
        //    if (product == null)
        //        throw new ArgumentNullException(nameof(product));
