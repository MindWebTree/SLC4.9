...
var AjaxCart = {
    ...
    init: function (usepopupnotifications, topcartselector, topwishlistselector, flyoutcartselector, flyoutwishlistselector,
        ...
        flyoutwishlistpopupselector, wishlist_deleteitem, wishlist_deleteitem_empty, localized_data, wishlist_additem, wishlist_movedtocart, cart_updateitem) {
        ...
        this.wishlist_additem = wishlist_additem;
        this.wishlist_movedtocart = wishlist_movedtocart;
        this.cart_updateitem = cart_updateitem;
    },
    ...
    success_process: function (response) {
        ...
        if (url_add != "") { 
            if (url_add.indexOf("remove-wishlist=") > 0) {
                ...
                $("[data-wishlistid='" + url_add.split("/")[3] + "']").attr("aria-label", "Add to wishlist");
                ...
                AjaxCart.wishlistNotification(response.updatetopwishlistsectionhtml);
            }
            ...
            else if (url_add.toLowerCase().indexOf("cartitem/delete") > 0) {
                AjaxCart.cartNotification(response.updatetopcartsectionhtml);
            }
            else if (url_add.toLowerCase().indexOf("cartitem/update") > 0) {
                AjaxCart.cartNotification(response.updatetopcartsectionhtml, "Update");
            }

            else {
                if (typeof response.Id !== "undefined") {
                    $("[data-wishlistid='" + url_add.split("/")[3] + "']").addClass("active");
                    $("[data-wishlistid='" + url_add.split("/")[3] + "']").attr("aria-label", "Product in wishlist");
                    $("[data-wishlistid='" + url_add.split("/")[3] + "']").attr("data-wishlist", response.Id);
                }
                if (url_add.toLowerCase().indexOf("iswishlist=true") > -1 && response.redirect == null && response.success == true) {
                    AjaxCart.wishlistNotification(response.updatetopwishlistsectionhtml, "Add");
                }
            }
        }
    ...
    wishlistNotification: function (wishlistCount, type) {
        ...
        if (type == "Add") {
            $("#" + id).html('<i class="fa fa-check" aria-hidden="true"></i>' + AjaxCart.wishlist_additem);
        }
        else if (type == "Moved") {
            $("#" + id).html('<i class="fa fa-check" aria-hidden="true"></i>' + AjaxCart.wishlist_movedtocart);
        }
        else {
    ...
    cartNotification: function (cartCount, type) {
        var noOfCartItems = 1;
        if (cartCount) {
            noOfCartItems = cartCount.replace("(", "");
            noOfCartItems = noOfCartItems.replace(")", "");
            noOfCartItems = parseInt(noOfCartItems);
        }
        var dt = new Date();
        var id = dt.getHours() + "-" + dt.getMinutes() + "-" + dt.getSeconds()
            + "-" + dt.getMilliseconds();
        ...
        $("body").append('<div class="alert alert-custom alert-dismissible fade  bootstrap-alert" id="' + id + '" role="alert"></div>');
        if (type == "Update") {
            $("#" + id).html('<i class="fa fa-check" aria-hidden="true"></i>' + AjaxCart.cart_updateitem);
        }
        else {

            $("#" + id).html('<i class="fa fa-check" aria-hidden="true"></i>' + (noOfCartItems > 0 ? AjaxCart.wishlist_deleteitem.replace("wish list", "cart") :
                AjaxCart.wishlist_deleteitem_empty.replace("wish list", "cart")));
        }
        $("#" + id).fadeTo(2000, 500).slideUp(300, function () {
            $("#success-alert").slideUp(100);
        });
